SELECT DISTINCT st.study_id as "study_id"
			...
			--the first load excludes deceased patients, the next one shall include them but FLAG them as such

						  ,CASE WHEN coh.EXCLUSION_REASON = 'patient deceased' THEN coh.EXCLUSION_DATE ELSE NULL END "death_date"
...
where 
	...
	--the first loads excludes deceased patients as well, the next load shall include them but FLAG them as such
